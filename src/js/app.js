...
function mostrarResumen() {
    ...
    // 🔄 Capturar valor actual del input antes de validar
    const inputNombre = document.getElementById('nombre');
    if (inputNombre) {
        const nombreActual = inputNombre.value.trim();
        cita.nombre = nombreActual;
    ...
    servicios.forEach(servicio => {
        ...
        textServicio.textContent = nombreServicio;
    ...
    const nombre1 = inputNombre.value.trim();

    if (nombre1) {
        resultado.innerHTML = `<span>Nombre:</span> ${nombre}`;
    } else {
        resultado.innerHTML = `<span style="color:red;">Por favor ingresa tu nombre.</span>`;
    }
